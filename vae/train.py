...
def ae_loss(model, x):
    ...
    ##################################################################
    reconstructed_x = model.decoder(model.encoder(x))
    loss = torch.mean(torch.sqrt(torch.norm(x-reconstructed_x)))
    ##################################################################
    #                          END OF YOUR CODE                      #
    ##################################################################
...
def vae_loss(model, x, beta = 1):
    ...
    # print(mu.shape)
    # print(log_std.shape)
    z = mu + torch.exp(log_std) * torch.randn(mu.size()).to('cuda')
    reconstructed_x = model.decoder(z)

    recon_loss = torch.mean(torch.norm(x-reconstructed_x))
    kl_loss = 0.5 * beta * torch.mean(-2*log_std + torch.exp(2*log_std) + mu**2 - 1)
...
def linear_beta_scheduler(max_epochs=None, target_val = 1):
    ...
    ##################################################################
    def _helper(epoch):
        beta = (target_val / max_epochs) * epoch
        return beta
    ##################################################################
    #                          END OF YOUR CODE                      #
    ##################################################################
